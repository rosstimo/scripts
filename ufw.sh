...
HOME_LAN_CIDR="192.168.0.0/16"   # Home network
...
syncthing_allow_on_cidrs() {
  ...
  for CIDR in "${SYNCTHING_ALLOWED_CIDRS[@]}"; do
    ufw allow from "$HOME_LAN_CIDR" to any port 22000 proto tcp &&
    ufw allow out to "$CIDR" port 22000 proto tcp &&
    ufw allow from "$HOME_LAN_CIDR" to any port 21027 proto udp &&
    ufw allow out to "$CIDR" port 21027 proto udp &&
    ufw allow from "$HOME_LAN_CIDR" to any port 22067 proto udp &&
    ufw allow out to "$CIDR" port 22067 proto udp &&
    echo
  done
  ...
  for IFACE in "${WORK_IFACES[@]}"; do
    ...
    ufw deny in on "$IFACE" to any port 22000 proto tcp || true &&
    ufw deny out on "$IFACE" to any port 22000 proto tcp || true &&
    ufw deny in on "$IFACE" to any port 22067 proto udp || true &&
    ufw deny out on "$IFACE" to any port 22067 proto udp || true &&
    echo
  done
}
